...
def qiandao(cookies):
    ...
    qiandao_login_url = (
        "http://h5.rrhuodong.com/index.php?g=api/mission&m=clock&a=store&id=2"
    )
    ...
    print(
        "{} 签到成功： 称昵：{} 等级：{} 人人钻：{} ".format(
            time.strftime("%Y-%m-%d %H:%M", time.localtime()),
            info["nickname"],
            info["main_group_name"],
            info["point"],
        )
    )
    headers.pop("Cookie")
...
def get_huodong(uid, token):
    huodong_login_url = "http://h5.rrhuodong.com/index.php?g=api/mission&m=index&a=login&uid={}&token={}".format(
        uid, token
    )
    return requests.utils.dict_from_cookiejar(get(huodong_login_url).cookies)[
        "PHPSESSID"
    ]


def get_token():
    token_login_url = "http://a.zmzapi.com/index.php?g=api/public&m=v2&accesskey=519f9cab85c8059d17544947k361a827&client=2&a=login&account={}&password={}".format(
        username, passwd
    )
